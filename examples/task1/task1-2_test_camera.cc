...
public:
    ...
    math::Vec2d projection(math::Vec3d const & p3d){
        double x_camera = R_[0] * p3d[0] + R_[1] * p3d[1] + R_[2] * p3d[2] + t_[0];
        double y_camera = R_[3] * p3d[0] + R_[4] * p3d[1] + R_[5] * p3d[2] + t_[1];
        double z_camera = R_[6] * p3d[0] + R_[7] * p3d[1] + R_[8] * p3d[2] + t_[2];

        double x_homo = x_camera / z_camera;
        double y_homo = y_camera / z_camera;

        double r2 = x_homo * x_homo + y_homo * y_homo;
        double distortion = 1 + dist_[0] * r2 + dist_[1] * r2 * r2;

        math::Vec2d p;
        p[0] = f_ * x_homo * distortion + c_[0];
        p[1] = f_ * y_homo * distortion + c_[1];
